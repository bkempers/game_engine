  ...
  for(int i = 0; i < chunk.chunk_size; i++)
    for(int j = 0; j < chunk.chunk_size; j++)
      for(int k = 0; k < chunk.chunk_size; k++)
        setPosition(glm::vec3(i, j, k), chunk.data[chunk.getIndex(glm::vec3(i,j,k))].type);
...
bool Octree::trySimplify(){
  ...
  for(unsigned int i = 0; i < subTree.size(); ++i){
    ...
    else if(subTree[i].type == VOXEL_AIR) subTree.erase(subTree.begin()+i);
...
bool Octree::setPosition(glm::vec3 _pos, Voxel_Type _type){
  ...
  */
  ...
  if(subTree.empty() && type != VOXEL_AIR){
    for(int i = 0; i < 8; i++){
      Octree element;
...
Voxel_Type Octree::getPosition(glm::vec3 _pos, int LOD){
